...
import confetti from 'canvas-confetti'; // --- THE FLOWERS TOOL ---
...
function App() {
  ...
  useEffect(() => {
    // If the level increased, fire the flowers!
  ...
  const upgradeToPro = async () => {
    try {
      ...
      alert("💎 Welcome to Pro! Analytics unlocked.");
    } catch (err) {
      console.log(err);
    }
  };
      ...
      <AnimatePresence>
        {showLevelUp && (
          <motion.div 
            ...
            style={{ position: 'fixed', top: '50%', left: '50%', x: '-50%', y: '-50%', zIndex: 2000, textAlign: 'center', pointerEvents: 'none' }}
          >
            <h1 style={{ fontSize: '4rem', color: '#f1c40f', textShadow: '0 0 20px rgba(0,0,0,0.5)', margin: 0 }}>LEVEL UP! 🎊</h1>
            <div style={{ backgroundColor: 'white', padding: '10px 30px', borderRadius: '50px', boxShadow: '0 10px 30px rgba(0,0,0,0.3)', display: 'inline-block' }}>
      ...
      <div className="dashboard" style={{ display: 'flex', flexWrap: 'wrap', gap: '20px', justifyContent: 'center' }}>
        <div className="main-content" style={{ flex: '1', minWidth: '300px', maxWidth: '600px' }}>
          ...
          <div className="player-stats" style={{ backgroundColor: '#2c3e50', color: 'white', padding: '15px', borderRadius: '8px', marginBottom: '20px', textAlign: 'left', boxShadow: '0 4px 6px rgba(0,0,0,0.1)' }}>
            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start' }}>
              <div style={{ display: 'flex', gap: '15px', alignItems: 'center' }}>
                <motion.img 
                  animate={showLevelUp ? { scale: [1, 1.3, 1], rotate: [0, 10, -10, 0] } : {}}
            ...
            <div className="xp-bar-container" style={{ backgroundColor: '#1a252f', height: '25px', borderRadius: '12px', position: 'relative', overflow: 'hidden', boxShadow: 'inset 0 2px 4px rgba(0,0,0,0.3)', marginTop: '15px' }}>
              <motion.div initial={{ width: 0 }} animate={{ width: `${xp}%` }} transition={{ type: 'spring', stiffness: 50 }} style={{ backgroundColor: '#f1c40f', height: '100%' }} />
              <span style={{ position: 'absolute', width: '100%', textAlign: 'center', top: '2px', fontSize: '14px', fontWeight: 'bold', color: xp > 50 ? '#2c3e50' : 'white' }}>
                {xp} / 100 XP
              </span>
            </div>
          </div>
          ...
          <div style={{ marginBottom: '20px' }}>
            {isPremium ? <ConsistencyChart habits={habits} /> : (
              <div className="pro-paywall" style={{ backgroundColor: '#2c3e50', padding: '30px', borderRadius: '8px', textAlign: 'center', color: 'white', border: '2px dashed #f1c40f' }}>
                <h3 style={{ color: '#f1c40f' }}>Unlock Analytics 📈</h3>
                <button onClick={upgradeToPro} style={{ backgroundColor: '#f1c40f', color: '#2c3e50', padding: '10px 20px', border: 'none', borderRadius: '5px', fontWeight: 'bold', cursor: 'pointer' }}>Upgrade to Pro 💎</button>
              </div>
            )}
          </div>
          ...
          <section className="forge-area" style={{ marginBottom: '20px', backgroundColor: '#f8f9fa', padding: '20px', borderRadius: '8px', border: '1px solid #e9ecef' }}>
            ...
            <form onSubmit={addHabit} style={{ display: 'flex', flexDirection: 'column', gap: '10px' }}>
              ...
              <button type="submit" className="forge-btn" style={{ padding: '10px', backgroundColor: '#3498db', color: 'white', border: 'none', borderRadius: '4px', fontWeight: 'bold', cursor: 'pointer' }}>Forge ⚒️</button>
            </form>
          </section>
          ...
          <section className="habit-display">
            <AnimatePresence>
              {habits.map(habit => (
                <motion.div key={habit._id} initial={{ opacity: 0, x: -20 }} animate={{ opacity: 1, x: 0 }} exit={{ opacity: 0, scale: 0.5 }} className="habit-card" style={{ borderLeft: `6px solid ${habit.color || '#3498db'}`, padding: '15px', backgroundColor: 'white', borderRadius: '8px', marginBottom: '10px', position: 'relative' }}>
                  <h4 style={{ margin: '0 0 10px 0' }}>{habit.icon} {habit.title}</h4>
                  <p>Streak: {habit.currentStreak} 🔥</p>
                  <motion.button whileTap={{ scale: 0.9 }} className="complete-btn" onClick={() => completeHabit(habit._id)} style={{ backgroundColor: '#2ecc71', color: 'white', border: 'none', padding: '8px 15px', borderRadius: '4px', cursor: 'pointer' }}>Done! ✅</motion.button>
                  <button onClick={() => deleteHabit(habit._id)} style={{ marginLeft: '10px', backgroundColor: '#ff4d4d', color: 'white', border: 'none', padding: '8px 15px', borderRadius: '4px', cursor: 'pointer' }}>Trash 🗑️</button>
                </motion.div>
              ))}
            </AnimatePresence>
          </section>
        ...
        <div className="multiplayer-sidebar" style={{ flex: '1', minWidth: '250px', maxWidth: '350px' }}>
          <motion.div initial={{ opacity: 0, x: 20 }} animate={{ opacity: 1, x: 0 }} className="profile-card" style={{ backgroundColor: '#fdfbf7', border: '2px solid #f1c40f', padding: '20px', borderRadius: '8px' }}>
            <h3 style={{ borderBottom: '2px solid #f1c40f', paddingBottom: '10px' }}>Hero Profile 🛡️</h3>
            <div style={{ textAlign: 'center' }}>
              ...
              <h4>{localStorage.getItem('userEmail') || 'Habit Forger'}</h4>
            </div>
